...
def banner():
    console.print(Panel.fit(
        "[bold cyan]⚡ ReconXpress[/bold cyan]\n[white]Advanced Reconnaissance Tool[/white]",
        subtitle="by Madhu", subtitle_align="right"
    ))
...
def main():
    ...
    # Normalize target name for filenames
    target_name = cleaned_input.replace(".", "_").replace("https://", "").replace("http://", "").replace("/", "")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    base_output_path = os.path.splitext(args.output)[0]
    final_html_path = f"{base_output_path}_{target_name}_{timestamp}.html"
    final_json_path = final_html_path.replace(".html", ".json")
    ...
    with Progress(
        ...
        for key, enabled in selected_modules.items():
            ...
            try:
                ...
                console.print(f"[red]❌ {key.upper()} scan failed: {e}[/red]")
            finally:
                progress.update(task_id, completed=1)

    # Generate one final report
    try:
        console.print("\n[bold yellow]📄 Generating final HTML report...[/bold yellow]")
        generate_html_report(cleaned_input, results, final_html_path)
        console.print(f"[bold green]✅ Final report saved to:[/bold green] {final_html_path}")
    except Exception as e:
        console.print(f"[red]❌ Failed to generate HTML report: {e}[/red]")

    if args.json:
        try:
            export_json(results, final_json_path)
            console.print(f"[bold blue]📁 JSON report saved to:[/bold blue] {final_json_path}")
        except Exception as e:
